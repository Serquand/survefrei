...
const SurveyField = (props: Props) => {
    const [value, setValue] = useState(props.field);
    const API_URL = import.meta.env.VITE_API_URL;
    const fieldTypeOptions: { label: string, id: SurveyFieldType }[] = [
    ...
    const positionOptions: { label: string, id: number }[] = Array.from(new Array(props.numberOfFields)).map((_, index) => ({
        id: index + 1,
        label: '' + (index + 1)
    }));
    ...
    const updateChoices = (newChoice: string, choiceIndex: number) => {
        ...
        setValue(prevState => {
            const newState = {
                ...prevState,
                choices: prevState.choices.map((choice, index) => index === choiceIndex ? newChoice : choice)
            };
        ...
        <>
            <div className="mx-auto flex flex-col gap-6">
                <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="flex flex-col gap-4">
                            {value.choices.map((_, index) => (
                                <div className="flex items-end gap-4">
                                    <div className="grow">
                                        <InputField
                                            modelValue={value.choices[index]}
                            ...
                            <button
                                type="button"
                                onClick={() => {
                                    updateFieldValue("choices", [...value.choices, ""]);
                                }}
                    ...
                    {value.fieldType === SurveyFieldType.SELECT ? <InputField
